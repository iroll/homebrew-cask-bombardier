class Bombardier < Formula
  desc "Classic GNU terminal bombing game"
  ...
  def install
    ...
    # Build and install
    system "make"
    bin.install "bombardier"
    man6.install "bombardier.6"

    # Create score directory
    (var/"games/bombardier").mkpath
  end
  ...
  def caveats
    <<~EOS
      ...
      To completely remove the game including high scores:
        brew uninstall --zap bombardier
    EOS
  end

  test do
    # Test that the binary exists and can print help/version info
    assert_match "Bombardier", shell_output("#{bin}/bombardier 2>&1", 1)
  end

  def zap
    rm_rf var/"games/bombardier"
  end
end